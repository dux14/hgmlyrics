...
describe('createPipelineRun', () => {
  it('POST con body {filename,size,mime} y header Bearer', async () => {
    ...
    const result = await createPipelineRun('s1', {
      fileName: 'a.mp3',
      size: 10,
      mime: 'audio/mpeg',
    });
...
describe('watchPipelineRun', () => {
  ...
  it('un evento de broadcast dispara un refresh (tras el debounce)', async () => {
    global.fetch.mockResolvedValue(jsonResponse(200, { run: { id: 'r1' } }));
    const onChange = vi.fn();
    watchPipelineRun('s1', onChange);
    await vi.advanceTimersByTimeAsync(0);
  ...
  it('coalesce: una ráfaga de broadcasts hace UN solo refresh', async () => {
    global.fetch.mockResolvedValue(jsonResponse(200, { run: { id: 'r1' } }));
    const onChange = vi.fn();
    watchPipelineRun('s1', onChange);
    await vi.advanceTimersByTimeAsync(0);
  ...
  it('detiene el polling cuando el run llega a estado terminal (done)', async () => {
    global.fetch.mockResolvedValue(jsonResponse(200, { run: { id: 'r1', status: 'done' } }));
    const onChange = vi.fn();
    watchPipelineRun('s1', onChange);
    await vi.advanceTimersByTimeAsync(0);
    expect(global.fetch).toHaveBeenCalledTimes(1);

    // Ya no debe seguir haciendo GET cada 3s: el run terminal no cambia solo.
    await vi.advanceTimersByTimeAsync(9000);
    expect(global.fetch).toHaveBeenCalledTimes(1);
  });

  it('sigue polleando mientras el run está activo (processing)', async () => {
    global.fetch.mockResolvedValue(jsonResponse(200, { run: { id: 'r1', status: 'processing' } }));
    const onChange = vi.fn();
    watchPipelineRun('s1', onChange);
    await vi.advanceTimersByTimeAsync(0);
    expect(global.fetch).toHaveBeenCalledTimes(1);
  ...
  it('un broadcast reactiva el polling tras un run terminal', async () => {
    global.fetch.mockResolvedValue(jsonResponse(200, { run: { id: 'r1', status: 'done' } }));
    const onChange = vi.fn();
    watchPipelineRun('s1', onChange);
    await vi.advanceTimersByTimeAsync(0);
    expect(global.fetch).toHaveBeenCalledTimes(1);
    await vi.advanceTimersByTimeAsync(9000);
    expect(global.fetch).toHaveBeenCalledTimes(1); // polling detenido

    // Arranca un run nuevo: broadcast reactiva el ciclo.
    global.fetch.mockResolvedValue(jsonResponse(200, { run: { id: 'r2', status: 'processing' } }));
    const broadcastHandler = channelStub.on.mock.calls.find(
      (call) => call[0] === 'broadcast' && call[1]?.event === 'change',
    )[2];
    broadcastHandler();
    ...
    expect(global.fetch).toHaveBeenCalledTimes(2);
    await vi.advanceTimersByTimeAsync(3000);
    expect(global.fetch).toHaveBeenCalledTimes(3); // polling de nuevo activo
  });
  ...
  it('guard reqId: un rechazo de un refresh obsoleto no emite {error}', async () => {
    ...
    await vi.advanceTimersByTimeAsync(400); // debounce del broadcast
