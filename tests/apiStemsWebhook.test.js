...
// ─── Nuevo contrato per-sección DAG ──────────────────────────────────────────
// El orquestador Modal postea { jobId, section, result } sin query params.
// La firma se verifica con verifyModalSignature (mismo helper que arriba).
...
function modalSectionReq(bodyObj) {
  ...
  req.method = 'POST';
  req.headers = { 'x-modal-timestamp': timestamp, 'x-modal-signature': sig };
  // Sin query params: nuevo contrato DAG
  req.query = {};
...
function setupSqlBegin({ sections, status = 'processing' } = {}) {
  ...
  // Reemplaza sqlMock.begin con una implementación funcional para esta prueba.
  // El callback recibe un sql interno que consume sqlResponses en orden.
  ...
  if (sections !== null) {
    // SELECT FOR UPDATE
    sqlResponses.push([{ sections, status }]);
    // UPDATE stem_jobs SET sections = ...
    sqlResponses.push([]);
  } else {
    // Job desconocido: SELECT FOR UPDATE retorna vacío
    sqlResponses.push([]);
  }
}
...
describe('POST /api/stems/webhook — contrato per-sección DAG', () => {
  beforeEach(() => {
    // Restaurar begin después de cada test de sección
    sqlMock.begin = undefined;
  });

  it('401 si la firma Modal es inválida (contrato por sección)', async () => {
  ...
  it('400 si section no es válida', async () => {
    const res = makeRes();
    await handler(modalSectionReq({ jobId: 'j1', section: 'SECCION_INVALIDA', result: { status: 'done' } }), res);
    expect(res.statusCode).toBe(400);
  });
